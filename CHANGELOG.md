...
- **Separación real de stack: nuevos plugins `flujo-dotnet` y `flujo-node`.** `flujo-core` dejó de traer comandos `dotnet` quemados en sus templates — no era tan agnóstico como se afirmaba. Se movió todo lo específico de .NET (`gauntlet.json` con 8/11 etapas dotnet, `stryker-config.json` con schema .NET, `ci/quality-gate.yml`, la sección `[*.cs]` de `.editorconfig`, los permisos `Bash(dotnet ...)`) a un plugin nuevo **`flujo-dotnet`** (pack de lenguaje). Se creó su espejo **`flujo-node`** (Vitest, Prettier, Playwright, Cucumber.js, Stryker JS) — antes no existía ningún equivalente para JS/TS, había que reescribir todo a mano.
- **Arquitectura de 3 capas:** core (universal) → stack-pack de lenguaje (`flujo-dotnet`/`flujo-node`) → stack-pack de framework (`flujo-devexpress` sobre `flujo-dotnet`; a futuro `flujo-vue`/`flujo-react` sobre `flujo-node`). `/flujo-init` ahora detecta qué pack de lenguaje está instalado y copia SUS templates; si no hay ninguno, genera un `gauntlet.json` mínimo (solo etapas agnósticas) y avisa explícitamente en vez de asumir dotnet en silencio.
- **Mutation testing (Stryker) sigue siendo obligatorio y ahora multi-stack**: bloqueo duro con `testing.enabled=true`, igual en `flujo-dotnet` (Stryker.NET) y `flujo-node` (StrykerJS) — no se trata distinto por lenguaje.
- **Fix de motor: `codeExtensions` configurable.** `spec-guard.ps1`/`mark-dirty.ps1`/`scan-comments.ps1` tenían la lista de extensiones de código hardcodeada en el script (mezclando `.cs`/`.razor` con `.js`/`.ts` a mano, sin `.vue`/`.jsx`/`.tsx`). Ahora se lee de `flujo.json > codeExtensions`: `flujo-core` trae un default universal (`.css/.scss/.sql/.ps1`) y cada stack-pack anexa las suyas al instalarse.
- **Fix: etapa `docdrift` dejaba de ser real.** Su comando era literalmente `echo docs-rewrite --check` — un `echo` nunca falla, así que esa etapa nunca podía fallar de verdad en ningún proyecto. Ahora es `internal: docdrift-check`, con un script real que detecta notas sin procesar en `docs/_inbox/`.
- **Limpieza de `flujo-devexpress`:** se retiró la skill `conexion-bases-datos` — no era XAF ni .NET, era infraestructura privada (IPs internas, rutas de credenciales) que no debía vivir en un plugin de marketplace. El pack pasa a depender documentalmente de `flujo-dotnet` para gauntlet/CI.
- Generalizada la prosa de `flujo-core` que asumía .NET fuera de los templates: menciones sueltas a `.cs`/Razor/XML docs/ViewControllers en `flujo-mode.md`, `workflow-plan.md`, `flujo-implement.md`, `design-critic.md`, `solid-guardian.md`, `ctx/SKILL.md`, `gauntlet/SKILL.md`.
- README: nueva sección de instalación con el pack de lenguaje como paso explícito (dotnet **o** node), tabla de herramientas Node equivalente a la de .NET, y corrección de la tabla del guantelete (`mutation` estaba documentada como `nightly/informativo`, pero el template real siempre fue `local/duro` — quedó documentada como lo que de verdad hace).
